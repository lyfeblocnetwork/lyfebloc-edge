...
build:
	...
	go build -o lyfebloc-edge -ldflags="\
    	-X 'github.com/lyfeblocnetwork/lyfebloc-edge/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/lyfeblocnetwork/lyfebloc-edge/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/lyfeblocnetwork/lyfebloc-edge/versioning.Branch=$(BRANCH)'\
		-X 'github.com/lyfeblocnetwork/lyfebloc-edge/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/lyfebloc-edge .
	env EDGE_BINARY=${PWD}/artifacts/lyfebloc-edge go test -v -timeout=30m ./e2e/...
