...
LYFEBLOC_EDGE_BIN=./lyfebloc-edge
...
case "$1" in
   ...
   "init")
      if [ -f "$GENESIS_PATH" ]; then
          ...
          secrets=$("$LYFEBLOC_EDGE_BIN" secrets init --num 4 --data-dir data- --json)
          echo "Secrets have been successfully generated"

          echo "Generating genesis file..."
          "$LYFEBLOC_EDGE_BIN" genesis \
            --dir "$GENESIS_PATH" \
            --consensus ibft \
   ...
   *)
      ...
      echo "Executing lyfebloc-edge..."
      exec "$LYFEBLOC_EDGE_BIN" "$@"
      ;;
